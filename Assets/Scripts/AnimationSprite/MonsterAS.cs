...
using UnityEngine;
[SelectionBase]
public class MonsterAS : MonoBehaviour , IDamagable
{

    [SerializeField] private MonsterStat _monsterStat;
    [SerializeField] private Rigidbody2D _rigidbody;
    [SerializeField] private SpriteRenderer _spriteRenderer;
    [SerializeField] private AnimationSpriteReader _animationSpriteReader;
    ...
    [Space(10),Header("So AnimationSprite")]
    [SerializeField] private SOAnimationSprite _asIdle;
    [SerializeField] private SOAnimationSprite _asWalk;
    [SerializeField] private SOAnimationSprite _asDamaged;
    [SerializeField] private SOAnimationSprite _asAttack;
    [SerializeField] private SOAnimationSprite _asDeath;
    ...
    private void ManageMove() {
        ...
        Debug.Log("play Walk");
        if (_animationSpriteReader)
        {
            
            _animationSpriteReader.PlayNewAnimationSprite(_asWalk);
        }
    ...
    private void ManageWaiting() {
        if (_animationSpriteReader)_animationSpriteReader.PlayNewAnimationSprite(_asIdle);
        if (CheckForPlayer()) {
            StartAttack();
            return;
        }
    ...
    private void StartAttack() {
        ...
        if (_animationSpriteReader)_animationSpriteReader.PlayNewAnimationSprite(_asAttack);
    ...
    public void TakeDamage(int damage, Vector2 origin) {
        ...
        if (_animationSpriteReader)_animationSpriteReader.PlayNewAnimationSprite(_asDamaged);
        _timer = 0;
        _monsterStat = MonsterStat.damaged;
        
        _hp--;
        if (_hp <= 0) {
            if (_animationSpriteReader)_animationSpriteReader.PlayNewAnimationSprite(_asDeath);
            this.enabled = false;
            if (_destroyOnDeath) {
